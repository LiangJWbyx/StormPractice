...
public class SentenceSpout extends BaseRichSpout {
    ...
    /**
     * open()方法是ISpout接口中定义，在Spout组件初始化时被调用
     * open()接受三个参数：一个包含Storm配置的Map，一个TopologyContext对象，提供了topology中
     *                     组件的信息,SpoutOutputCollector对象提供发射tuple的方法。
     ...
     * 用于告诉Storm流组件将会发出哪些数据流，每个流的tuple将包含的字段
     * @param outputFieldsDeclarer
     */
    public void declareOutputFields(OutputFieldsDeclarer outputFieldsDeclarer) {
        outputFieldsDeclarer.declare(new Fields("sentence"));//告诉组件发出的数据流将包含sentence字段
    }
}
