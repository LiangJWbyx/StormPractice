...
/**
 * 订阅split sentence bolt的输出流，实现单词计数，并发送当前计数给下一个bolt
 */
public class WordCountBolt extends BaseRichBolt {
    ...
    /**
     * 大部分实例变量通常是在prepare()中实例化，这个设计模式是由topology的部署方式决定的
    ...
    /**
     * 在execute()方法中，我们查找的收到的单词计数（如果不存在，初始化围为0）
     * 然后增加计数并存储，发出一个新的词和当前计数组成的二元组；
     * 发射计数作为流允许拓扑的其它bolt订阅和执行额外的处理
     * @param tuple
     */
